...
- Location: C:\Repo\bg-js-gaming-automation\tests\API-Tests\store.api.spec.js:31:7
...
```ts
  ...
  10 |   let createdOrder;
  11 |
  12 |   test.beforeAll('Store Service - Add a new order', async () => {    
  ...
  32 |     try {
  33 |       await StoreService.getOrderNegativeResponse(createdOrder.id);
